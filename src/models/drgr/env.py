...
class Env(gym.Env):
    ...
    def __init__(self, config: DRGR_Config, rating_matrix: csr_matrix, dataset_name: str):
        """
        Initialize Env
    ...
    def load_env(self):
        ...
        if not os.path.exists(self.env_path):
            env_model = NMF(n_components=self.config.env_n_components, init='random', tol=self.config.env_tol,
                            max_iter=self.config.env_max_iter, alpha_W=self.config.env_alpha, alpha_H=self.config.env_alpha, verbose=True,
                            random_state=0)
            print('-' * 50)
